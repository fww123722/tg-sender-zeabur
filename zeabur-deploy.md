...
一套系统只有一个 `OWNER_ID`（主人），可以拉若干**操作员**共用号池干活。现在操作员与主人同权（仅增删操作员除外）。
...
| `/addop <user_id> [备注名]` | 仅主人 | 授权，立即生效 |
| `/dropop <user_id>` | 仅主人 | 停用（审计记录保留） |
| `/oplist` | 仅主人 | 看当前白名单 |
...
| 举报中心全套 | ✅ | ✅ |
| 数据看板、操作审计 /oplog（看全量） | ✅ | ✅ |
| **增删操作员** `/addop` `/dropop` `/oplist` | ✅ | ❌ 仅主人 |

> 唯一保留给主人的是「增删操作员」——否则操作员能自己拉人扩权、
> 甚至把主人踢出白名单。这一条我建议永远不要开。
