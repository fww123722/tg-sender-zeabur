...
# ---- 名单占用（targets 表是全库共用的，多人同时跑会互相覆盖）----
...
def campaign_text(uid=None) -> str:
    """生成群发运营的当前进度文本（已不选群：写文案 → 确认开跑 两步）。"""
    c = get_campaign(uid)
    if not c:
        return ("尚未开始。点「① 写文案」写推广文案 → 点「② 确认开跑」开始群发"
                "（默认打所有群的人，按录入时间从新到旧）。")
    lines = ["【群发运营】当前进度："]
    checks = {
    ...
    lines.append(f"  当前: 名单={c.get('target_count', 0)}人 | "
                 f"文案={'已填' if c.get('text') else '未填'}")
    if checks.get("text"):
        lines.append("→ 就绪，点「② 确认开跑」开始群发")
    else:
        lines.append(f"→ 已完成 {done}/1 步")
    return "\n".join(lines)
